...
# an asset is compliant when its capture time is set in exif, its filename already
# follows the enforced YYYYMMDD-HHMMSS convention on a normalized extension, and
# its fs modification timestamp tells the same wall-clock time: apto would rewrite
# it to identical values. skipped untouched and left in place, it pins its slot in
# the sequence and the shifting in install_media_file routes renamed files around
# it, which makes it an anchor for the rest of the pool
function asset_compliant() {
	local fname="$1" exif_timestamps="$2" basename stem ext
	local exif_create_date_raw exif_create_date exif_offset fs_modification_time
	basename="$(basename "${fname}")"
	stem="${basename%.*}"
	[[ "${stem}" =~ ^[0-9]{8}-[0-9]{6}$ ]] || return 1
	# the enforced name ends in a lowercase, reduced extension (jpeg folds to jpg)
	ext="$(echo "${basename##*.}" | tr '[:upper:]' '[:lower:]' | sed 's/jpeg/jpg/')"
	...
	# time in Date/Time Original, so fall back to it
	...
	# no usable exif capture time, so there is nothing to be compliant against
	[ -n "${exif_create_date_raw}" ] || return 1
	exif_create_date="$(strip_offset "${exif_create_date_raw}")"
	[ "${#exif_create_date}" = 16 ] && exif_create_date="${exif_create_date}:00"
	# an exif time declaring a zone other than the target one would be shifted by
	# the tz conversion below, so the file would not hold its slot unchanged
	...
	if [ -n "${exif_offset}" ] && [ "$(normalize_tz "${exif_offset}")" != "${TARGET_TZ}" ]; then
		return 1
	fi
	# the fs mtime must tell the same wall-clock time the exif holds. the filename's
	# own digits are not compared against it: collision shifting can legitimately
	# leave a compliant asset a few seconds away from its exif time
	fs_modification_time="$(strip_offset "$(awk -F': ' '/^File Modification Date\/Time  /{print $2}' <<<"${exif_timestamps}" | head -1)")"
	[ "${fs_modification_time}" == "${exif_create_date}" ] || return 1
	return 0
}
...
while read -r fname <&3; do
	...
	# parse timestamps. QuickTimeUTC makes exiftool expose mp4/mov dates (stored as
	# utc by spec) with an explicit offset instead of a naive value we'd misread as
	# local. read once and share between the compliance probe and the mode logic
	exif_timestamps=""
	if [ "${MODE}" != "static" ] || [ "${SKIP_COMPLIANT}" = 1 ]; then
		exif_timestamps="$(exiftool -api QuickTimeUTC=1 -time:all "${fname}")"
	fi

	if [ "${SKIP_COMPLIANT}" = 1 ] && asset_compliant "${fname}" "${exif_timestamps}"; then
		echo "Compliant ${basename}: skipping, keeping it as anchor"
		continue
	fi
