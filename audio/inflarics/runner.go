	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, path := range args {
				...
				artist := sys.Fallback(sys.ErrWrap(tag.Artist())(cmd.Flags().GetString("artist")), tag.Artist())
				if len(artist) == 0 && len(uslt) == 0 {
					return errors.New("track artist is mandatory")
				}

				title := sys.Fallback(sys.ErrWrap(tag.Title())(cmd.Flags().GetString("title")), tag.Title())
				if len(title) == 0 && len(uslt) == 0 {
					return errors.New("track title is mandatory")
				}

				if len(uslt) == 0 {
					lyrics, lyricsErr := getLyrics(artist, title, sys.ErrWrap("")(cmd.Flags().GetString("url")), tag)
					if lyricsErr != nil {
						return lyricsErr
					}
					uslt = lyrics
