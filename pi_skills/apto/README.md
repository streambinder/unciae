...
`--skip-compliant` makes a trusted asset that is already fully compliant —
exif time set, `YYYYMMDD-HHMMSS.ext` filename, fs timestamp matching the exif
time — exit untouched: no rename, no `touch`, no rewrite. Left exactly where it
is, it pins its slot and anchors the sequencing of the pool: an asset renamed
onto an occupied timestamp shifts a second forward instead of displacing it.
