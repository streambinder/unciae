...
function mmkdir() {
	awk -F'/' '{ for(i = 1; i <= NF; i++) { print $i; } }' <<< "$1" | grep -v ^$ | while read col; do
		subpath="${subpath}/${col}"
...
function mmtopemptydir() {
	...
	if [ "$(megals -R "${dir_parent}" | grep -v "^${dir_child}" | wc -l)" -gt 1 ]; then
		echo "${dir_child}"
	else
...
if [ ! -f "${DIR_OFFLINE}/mailserver.tar.xz" ]; then
	...
	cd /var/vmail
	tar cvfJ "${DIR_OFFLINE}/mailserver.tar.xz" $(grep -v mailfilters <<< "$(ls)" | xargs)
fi
...
if [ ! -z "${BACKUP_EXT_FOLDERS}" ]; then
        for folder in ${BACKUP_EXT_FOLDERS}; do
                ...
                cd "${folder}"
                tar cvfJ "${DIR_OFFLINE}/${folder_bkp}" $(find -maxdepth 1 -not -name '.' -not -name '..' -printf "%f ")
                cd -
        done
fi
...
while read -r backup_path; do
	backup="$(sed "s|${BACKUP_MEGADIR}||g" <<< "${backup_path}" | cut -c 2- | sed 's|/|-|g')"
...
done <<< "$(megals -R "${BACKUP_MEGADIR}" | egrep -e "^${BACKUP_MEGADIR}/[0-9]{4}/[0-9]{2}/[0-9]{2}$" | sort -u)"
...
done <<< "$(megals -R "${BACKUP_MEGADIR}" | egrep /[0-9]+$)"
...
if [ ! -z "${CONTACT_EMAIL}" ]; then
	cat "${LOG_FILE}" | mail -s "MEGA backup outcome" "${CONTACT_EMAIL}"
fi
